...
from .llm_manager import LLMManager, LLMConfig, GenerationMetrics, create_model_configs
from .prompt_engine import PromptEngine, QueryType, ContextInfo, ContextType
...
class LegalResponseSystem:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            # بارگذاری مدل پیش‌فرض (mock)
            success = self.llm_manager.load_model_mock(self.default_model)
            if not success:
                logger.error(f"خطا در بارگذاری مدل پیش‌فرض: {self.default_model}")
                return False
...
class SimpleLegalAssistant:
    ...
    def __init__(self):
        self.system = LegalResponseSystem()
    ...
    async def ask(
        ...
        if not self.initialized:
            return "سیستم راه‌اندازی نشده است"
        ...
        result = await self.system.generate_response(request)
        
        if result.success:
            return result.enhanced_response
        else:
            return f"خطا: {result.error_message}"
