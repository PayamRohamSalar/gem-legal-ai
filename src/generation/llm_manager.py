...
class OllamaManager:
    ...
    def _check_connection(self) -> bool:
        ...
        try:
            ...
            # کد اصلاح شده برای جلوگیری از خطا
            self.available_models = [
                model['name'] for model in response.get('models', []) if 'name' in model
            ]
            self.connection_status = True
            logger.info(f"اتصال به Ollama برقرار شد. مدل‌های موجود: {self.available_models}")
            return True
        except Exception as e:
            logger.error(f"خطا در اتصال به Ollama: {e}")
            self.connection_status = False
            return False
    ...
    def pull_model(self, model_name: str) -> bool:
        ...
        try:
            logger.info(f"شروع دانلود مدل: {model_name}")
